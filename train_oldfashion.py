...
# images_dir_train = dirPath + 'segnet/data/yeast/originaltrainimages/'
# labels_dir_train = dirPath + 'segnet/data/yeast/originaltrainsegm/'

images_dir_train = dirPath + 'segnet/data/yeast/fakeDataLarge/fakeimages/'
labels_dir_train = dirPath + 'segnet/data/yeast/fakeDataLarge/fakesegmCorrected/'
...
trX,valX,teX,trY,valY,teY = read_data(images_dir_train,labels_dir_train,images_dir_val,labels_dir_val,images_dir_test,labels_dir_test)
...
tf.app.flags.DEFINE_integer('batch', 50, 'Batch size')
tf.app.flags.DEFINE_integer('steps', 25, 'Number of training iterations') # was 2
...
with tf.Session(config=session_config) as sess:
  ...
  f = open(dirPath + 'segnet/train_logs/' + time_str + '.txt','w', 0)
    ...
    for i in range(int(trX.shape[0]/FLAGS.batch)):
      ...
      if max_traing_accuracy < current_accuracy:
        ...
        for count in range(0,FLAGS.batch):
          ...
          cv2.imwrite(dirPath + 'segnet/train_logs/' + str(count) + "_training.png", np_merge)
    ...
    if mean_val_accuracy > max_val_accuracy: #and abs(mean_val_accuracy - current_val_accuracy) < 0.1:#step % FLAGS.batch == 0:
      ...
      for i in range(int(teX.shape[0] / FLAGS.batch)):
        ...
        for count in range(0,FLAGS.batch):
          ...
          np_merge[: , 80*2 + 10 : 80*2 + 20] = 224 
          np_merge[: , 80*2 + 20: 80*3 + 20] = np_image
          cv2.imwrite(dirPath + 'segnet/train_logs/' + str(count) + "_testing.png", np_merge)
  ...
  # f = open(dirPath + 'segnet/train_logs/'+str(budget)+"_"+time_str+'.txt','w')
