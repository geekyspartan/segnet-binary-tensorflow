...
def imageRead():
    ...
    os.chdir(rootDir + "fakeDataLarge/fakesegm/")
    list_of_seg = sorted(glob.glob('*.png'))

    segmeFolder = rootDir +"fakeDataLarge/fakesegmCorrected/"

    for count in range(len(list_of_seg)):
        segImage = cv2.imread(rootDir + "fakeDataLarge/fakesegm/" + list_of_seg[count])
